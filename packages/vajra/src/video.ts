...
export function videoStreamer(options: VideoStreamerOptions): Middleware {
  ...
  // Probe ffmpeg + ffprobe at startup so the install gap surfaces immediately
  (async () => {
    ...
    if (missing.length > 0) {
      console.error(
        `[Vajra Video] Missing required binaries in PATH: ${missing.join(', ')}\n` +
        '  Install FFmpeg:\n' +
        '    Debian/Ubuntu:  sudo apt install ffmpeg\n' +
        '    Arch:           sudo pacman -S ffmpeg\n' +
        '    macOS:          brew install ffmpeg\n' +
        '    Windows:        choco install ffmpeg  (or https://ffmpeg.org/download.html)\n' +
        '  Video streaming will fail on every request until these are available.',
      );
    }
  })();

  // Init GPU detection (requires ffmpeg; silently no-op if missing)
  if (options.autoDetectGPU !== false) {
    detectGPU().then(gpu => {
      console.log(`[Vajra Video] GPU encoder: ${gpu.encoder}${gpu.nvenc ? ' (NVENC)' : gpu.qsv ? ' (QSV)' : ''}`);
    }).catch(() => {/* ffmpeg missing; already reported above */});
  }
