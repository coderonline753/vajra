...
export function imageProcessor(options: ImageProcessorOptions): Middleware {
  ...
  // Probe Sharp at startup so config errors surface before the first request
  (async () => {
    try {
      await import('sharp');
    } catch {
      console.error(
        '[Vajra Image] Sharp is not installed. Image processing will fail on every request.\n' +
        '  Install: bun add sharp\n' +
        '  Or remove imageProcessor() from your middleware stack.',
      );
    }
  })();
