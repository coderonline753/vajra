...
/** Internal shape exposed by Zod runtime. Public API receives ZodTypeAny. */
...
type ZodRuntime = { _def: ZodInternalDef } & ZodTypeAny;
...
function zodToJsonSchema(schema: ZodTypeAny | undefined): Record<string, unknown> {
  if (!schema || !(schema as ZodRuntime)._def) return {};

  const def = (schema as ZodRuntime)._def;
  ...
  switch (typeName) {
    ...
    case 'ZodObject': {
      ...
      const shapeFn = def.shape as (() => Record<string, ZodTypeAny>) | Record<string, ZodTypeAny> | undefined;
      const shape = typeof shapeFn === 'function' ? shapeFn() : shapeFn;
      for (const [key, value] of Object.entries(shape || {})) {
        properties[key] = zodToJsonSchema(value);
        const inner = (value as ZodRuntime)?._def?.typeName;
    ...
    case 'ZodDiscriminatedUnion':
      return { oneOf: [...((def.options as Map<string, ZodTypeAny> | ZodTypeAny[]) || [])].map((o) => zodToJsonSchema(o as ZodTypeAny)) };
    ...
    case 'ZodTuple':
      return {
        type: 'array',
        items: (def.items || []).map((i) => zodToJsonSchema(i)),
...
function generateSpec(options: OpenAPIOptions): Record<string, unknown> {
  ...
  for (const route of routes) {
    ...
    // Path params
    if (route.params) {
      const paramsDef = (route.params as ZodRuntime)?._def;
      const paramsShape = paramsDef?.shape;
      const shape = (typeof paramsShape === 'function' ? paramsShape() : paramsShape) || {};
      for (const [name, schema] of Object.entries(shape as Record<string, ZodTypeAny>)) {
    ...
    // Query params
    if (route.query) {
      const queryDef = (route.query as ZodRuntime)?._def;
      const queryShape = queryDef?.shape;
      const shape = (typeof queryShape === 'function' ? queryShape() : queryShape) || {};
      for (const [name, schema] of Object.entries(shape as Record<string, ZodTypeAny>)) {
        const isOptional = (schema as ZodRuntime)?._def?.typeName === 'ZodOptional';
        (operation.parameters as unknown[]).push({
